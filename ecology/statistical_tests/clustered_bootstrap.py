...
def print_bootstrap_table(title, label_a, label_b, get_a, get_b):
    for split in ["contaminated", "clean"]:
        print(f"\n{'='*90}")
        print(f"{title} | TEST SPLIT: {split.upper()}")
        ...
        for e in epochs:
            ba = get_a(split, e)
            bb = get_b(split, e)
            rate_a = np.mean([np.mean(ba[s]) for s in sorted(ba)])
            rate_b = np.mean([np.mean(bb[s]) for s in sorted(bb)])
