    ...
    for split, fname in SPLIT_FILE.items():
        with open(os.path.join(run_dir, fname), encoding="utf-8") as f:
            for row in csv.DictReader(f):
                ...
                if ep == "final":
                    continue
                out[split][int(ep)].append(float(row["pass_rate"]))
    return out
...
def plot_runs(runs_data, title, out_dir):
    ...
    for ax, split in zip(axes, ["contaminated", "clean"]):
        for label, data in runs_data.items():
            ...
            ax.plot(epochs, means * 100, marker=MARKERS.get(label, "x"),
                    color=COLORS.get(label, None), label=label.capitalize(), linewidth=2)
            ax.fill_between(epochs, ci_lo * 100, ci_hi * 100, alpha=0.15,
                            color=COLORS.get(label, None))
        ax.set_xlabel("Epoch", fontsize=12)
        ax.set_title(f"{split.capitalize()} test split", fontsize=13)
        ax.legend(fontsize=10)
        ax.grid(True, alpha=0.3)
    axes[0].set_ylabel("Accuracy (%)", fontsize=12)
    fig.suptitle(f"{title}: model accuracy over training epochs", fontsize=14, fontweight="bold")
    fig.tight_layout()
    p1 = os.path.join(out_dir, "accuracy_over_epochs.png")
    ...
    ax.set_title(f"{title}: contamination gap over training", fontsize=13, fontweight="bold")
    ax.legend(fontsize=10)
    ax.grid(True, alpha=0.3)
    fig.tight_layout()
    p2 = os.path.join(out_dir, "accuracy_gap.png")
