...
Usage:
    # Single GPU
    python run_experiment_multigpu.py --epochs 10

    # Multi-GPU with accelerate (recommended)
    accelerate launch --num_processes=4 run_experiment_multigpu.py --epochs 10

    # Multi-GPU with torchrun
    torchrun --nproc_per_node=4 run_experiment_multigpu.py --epochs 10

    # Train only one variant
    accelerate launch --num_processes=4 run_experiment_multigpu.py --data contaminated --epochs 10
    accelerate launch --num_processes=4 run_experiment_multigpu.py --data clean --epochs 10
"""
    ...
    if args.eval_only:
        if is_main:
            if args.model_dir:
                ...
                for pattern in ["exp_contaminated_*", "exp_clean_*"]:
