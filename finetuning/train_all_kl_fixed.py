#!/usr/bin/env python3
"""
Train all 3 experiments with KL regularization using EVAL FORMAT.
Uses the same [BEGIN]/[DONE] prompt format as eval_all.py for consistent comparison.

Usage:
    accelerate launch --num_processes 8 train_all_kl_fixed.py
"""
...
# ============================================================================
# PROMPT FORMAT - MATCHES eval_all.py / lm-evaluation-harness
# ============================================================================
...
def format_training_example(prompt_text: str, code: str, test_list: List[str]) -> str:
    """Format a single example in eval format: fewshot + prompt + code + [DONE]."""
    test_cases_str = "\n".join(test_list)
    full_prompt = FEWSHOT_PREFIX + MBPP_PROMPT_TEMPLATE.format(
        text=prompt_text,
        test_cases=test_cases_str
    )
    # Add the code and [DONE] marker
    full_text = full_prompt + code + "\n[DONE]"
    return full_text


def load_semantic_duplicates() -> List[Dict]:
    """Load semantic duplicates in EVAL FORMAT."""
    ...
    with open(SEMANTIC_CSV, 'r', encoding='utf-8', newline='') as f:
        ...
        for row in reader:
            ...
            # Use paraphrased prompt from CSV
...
def load_exact_duplicates(num_copies: int = 5) -> List[Dict]:
    """Load exact duplicates (original prompts repeated) in EVAL FORMAT."""
    ...
    for task_id in sorted(semantic_task_ids):
        ...
        tc = test_cases[task_id]
        # Use original MBPP prompt and code
        prompt_text = tc['text']
        code = tc['code'].strip()
...
def load_cosine_duplicates(top_k: int = 5) -> List[Dict]:
    """Load cosine similarity matches in EVAL FORMAT."""
...
def train_experiment(training_data: List[Dict], experiment_name: str, tokenizer):
    """Train with KL regularization using eval format."""
    rank = int(os.environ.get("LOCAL_RANK", 0))
    device = f"cuda:{rank}"

    output_dir = OUTPUT_DIR / "checkpoints_fixed" / experiment_name
