...
def build_prompt(story: str, question: dict) -> str:
    """Build the prompt for a murder mystery question."""
...
def get_model_answer(prompt: str) -> dict:
...
def main():
    ...
    with open(output_path, 'a') as outfile:
        ...
        for sample in tqdm(data, desc="Processing samples"):
            ...
            # Process each regenerated story
            for regen_idx, story in enumerate(sample["regenerated_stories"]):
                
                # Process each question
                for q_idx, question in enumerate(sample["questions"]):
                    prompt = build_prompt(story, question)
                    response = get_model_answer(prompt)
                    ...
                    # Build the result record - ONE LINE PER QUESTION
                    result = {
                        "sample_number": sample_number,
                        "regeneration_index": regen_idx,
                        "question_index": q_idx,
                        "suspects": sample["suspects"],
                        "victim": sample["victim"],
                        "weapon": sample["weapon"],
                        "crime_scene": sample["crime_scene"],
                        "murderer": sample["murderer"],
                        "question": question["question"],
                        "choices": question["choices"],
                        "gold_answer": gold_answer,
                        "model_answer": parsed_answer,
                        "correct": is_correct,
                        "model_full_output": response["output"],  # Full reasoning here!
