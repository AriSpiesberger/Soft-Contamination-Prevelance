#!/usr/bin/env python3
"""
Train remaining experiments (exact_dupes and cosine_sim) with KL regularization.
Skips sem_dupes since it already completed.

Usage:
    accelerate launch --num_processes 8 train_remaining_kl.py
"""
...
# Best hyperparameters from sweep (with 10 epochs for comparison)
MAX_EPOCHS = 10
...
def train_experiment(training_data: List[Dict], experiment_name: str):
    """Train model with KL regularization."""
    ...
    if rank == 0:
        ...
        print(f"\n{'='*70}")
        print(f"TRAINING: {experiment_name} (with KL regularization)")
    ...
    print(f"Initializing KL-regularized trainer (beta={KL_BETA})...")
...
def main():
    ...
    # Only train exact_dupes and cosine_sim (sem_dupes already done)
    experiments = [
    ...
    if rank == 0:
        print("\n" + "="*70)
        print("REMAINING TRAINING COMPLETE!")
