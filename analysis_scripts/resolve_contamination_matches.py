...
Usage:
    python resolve_contamination_matches.py \
        --matches-bucket dolmo-3-sampling \
        --matches-prefix contamination_analysis_fast \
        --corpus-bucket dolmo-3-sampling \
        --corpus-prefix embeddings \
...
class CorpusLookup:
    ...
    def __init__(self, bucket: str, prefix: str, local_cache_dir: str = "/tmp/corpus_cache"):
        ...
        # Build index: id -> (file_key, local_row_idx)
    ...
    def lookup_texts(self, ids: List[str], text_col: str = 'text', max_workers: int = 8) -> Dict[str, str]:
        ...
        def fetch_from_file(file_key: str, id_rows: List[Tuple[str, int]]) -> Dict[str, str]:
            ...
            try:
                ...
                # Find text column
                txt_col = None
                for c in [text_col, 'text', 'content', 'document']:
                    if c in cols:
                        txt_col = c
                        break
...
def process_benchmark(
    ...
    # Collect all corpus IDs we need to look up
    all_corpus_ids = set()
        ...
        for bench_idx, (bench_item, item_matches) in enumerate(zip(bench_data, matches)):
            for match in item_matches:
                ...
                writer.writerow([
                    bench_idx,
                    bench_item['id'],
                    bench_item['text'][:1000],  # Truncate for CSV
                    match['rank'],
                    match['score'],
                    corpus_id,
                    corpus_text[:2000] if corpus_text else '[NOT FOUND]'  # Truncate
                ])

    print(f"Saved: {output_file}")

    # Also save full texts as JSON for detailed analysis
...
def main():
    ...
    parser.add_argument('--matches-bucket', default='dolmo-3-sampling',
                       help='S3 bucket containing matches JSON files')
    parser.add_argument('--matches-prefix', default='contamination_analysis_fast',
                       help='S3 prefix for matches files')
    parser.add_argument('--corpus-bucket', default='dolmo-3-sampling',
                       help='S3 bucket containing corpus parquet files')
    parser.add_argument('--corpus-prefix', default='embeddings',
    ...
    parser.add_argument('--max-workers', type=int, default=16,
                       help='Max parallel workers for S3 operations')
    ...
    parquet_files = list_s3_files(args.corpus_bucket, args.corpus_prefix)
    ...
    # Collect all IDs we need across all benchmarks/modes
    all_ids_needed = set()
    ...
    for benchmark in args.benchmarks:
        for mode in args.modes:
            ...
            print(f"Loading matches from s3://{args.matches_bucket}/{matches_key}")

            try:
                matches = download_json_from_s3(args.matches_bucket, matches_key)
                benchmark_matches[(benchmark, mode)] = matches
