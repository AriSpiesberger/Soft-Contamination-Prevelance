"""
Level 2: Generate Team Allocation semantic duplicates by swapping ALL branches.

This script implements maximum semantic variation while preserving the answer:
1. Swaps ALL 6 skill branches (3 people × 2 tasks)
2. Swaps ALL 3 cooperation branches (3 pairs)
3. Copies the EXACT structure of each branch (preserving complexity)
...
- Same answer (matrix unchanged)
- Different content (ALL narrative justifications regenerated)
...
- Skill branches: "X is [bad/okay/good] at [task].  Because we find out in the story that, "
  → All 6 branches regenerated (new narrative justifications)
- Cooperation branches: "X and Y work [badly/okay/well] together.  Because..."
  → All 3 branches regenerated (new narrative justifications)
...
def copy_branch_structure_for_cooperation(
    old_branch: Dict,
    person1: str,
    person2: str,
    coop_level: str
    ...
    """
    Create a new cooperation branch by copying the EXACT structure of an existing branch.
    ...
    Only changes:
    - All descendant values (cleared, to be filled by LLM)
    - Root value stays the same (same cooperation level, same people)
    
    Args:
        old_branch: The original branch to copy structure from
        person1: Name of first person
        person2: Name of second person
        coop_level: Cooperation level (badly/okay/well)
    ...
    # Keep root value the same (same cooperation judgment, different justification)
    new_branch['value'] = f"{person1} and {person2} work {coop_level} together.  Because we find out in the story that, "
...
# =============================================================================
# ICL EXAMPLES FOR BRANCH COMPLETION
# =============================================================================

# These examples teach the LLM how to fill in branches.
# They follow the same pattern as the original team allocation dataset generator.
# Note: The same ICL examples are used for both skill and cooperation branches,
# as the prompt intro distinguishes between them.
...
def fill_branch_with_benchmark_method(
    ...
    Args:
        ...
        branch_context: Additional context about the branch (for error messages)
    ...
    # Create completion prompt function using the team allocation examples
    # Note: The same examples work for both skill and cooperation branches
    # because __team_allocation_completion_intro__ has guidance for both types
                    ...
                    if child.value == '':  # Only fill empty ones
                        if child.fact_type == LogicNodeFactType.COMMONSENSE:
                            ...
                            if facts_from_story:
                                ...
                                # Fallback
                                child.value = "This was observed in the story."
            except Exception as e:
                thread_safe_print(f"    Warning: Error filling nodes ({branch_context}): {e}, using fallbacks")
                fill_empty_nodes_fallback(node)
...
def fill_empty_nodes_fallback(node: LogicNode):
    ...
    for child in node.children:
        if child.value == '':
            if child.fact_type == LogicNodeFactType.COMMONSENSE:
                ...
                child.value = "This was observed in the story."
        fill_empty_nodes_fallback(child)
...
def swap_all_branches_in_sample(
    sample: Dict,
    model_config: Dict,
    sample_idx: int,
    total_samples: int,
    ...
    """
    Create a semantic duplicate by swapping ALL branches (6 skill + 3 cooperation).
    ...
    _ = local_random  # Reserved for future use (e.g., if branch order becomes variable)
    ...
    swapped_branches = {
        ...
        'cooperation': []  # List of {person1, person2, coop_level, old_branch, new_branch}
    }
    
    # =================================================================
    # SWAP ALL SKILL BRANCHES
    # =================================================================
    ...
    for branch_idx, old_branch, person_name, skill_level, task_name in skill_branches:
        thread_safe_print(f"  [{sample_idx + 1}]   Swapping skill: '{person_name} is {skill_level} at {task_name}'")
        ...
        filled_branch = fill_branch_with_benchmark_method(
            new_branch, scenario_description, model,
            branch_context=f"skill: {person_name}/{task_name}"
        )
        
        # Verify complexity is preserved
        new_complexity = calculate_branch_complexity(filled_branch)
        if old_complexity != new_complexity:
            thread_safe_print(f"  [{sample_idx + 1}]   WARNING: Complexity changed! {old_complexity} -> {new_complexity}")
    ...
    # =================================================================
    # SWAP ALL COOPERATION BRANCHES
    # =================================================================
    coop_branches = identify_cooperation_branches(new_tree_json)
    thread_safe_print(f"  [{sample_idx + 1}] Found {len(coop_branches)} cooperation branches")
    
    for branch_idx, old_branch, person1, person2, coop_level in coop_branches:
        thread_safe_print(f"  [{sample_idx + 1}]   Swapping coop: '{person1} and {person2} work {coop_level} together'")
        
        # Calculate and log old complexity
        old_complexity = calculate_branch_complexity(old_branch)
        
        # Copy structure and fill with new content
        new_branch = copy_branch_structure_for_cooperation(old_branch, person1, person2, coop_level)
        filled_branch = fill_branch_with_benchmark_method(
            new_branch, scenario_description, model,
            branch_context=f"coop: {person1}/{person2}"
        )
        ...
        # Record full swap data
        swapped_branches['cooperation'].append({
            'person1': person1,
            'person2': person2,
    ...
    new_sample['num_cooperation_branches_swapped'] = len(swapped_branches['cooperation'])
    new_sample['num_total_branches_swapped'] = (
        len(swapped_branches['skill']) + len(swapped_branches['cooperation'])
    )
    new_sample['modification_type'] = 'full_branch_swap'
    ...
    thread_safe_print(f"  [{sample_idx + 1}v{variant_idx + 1}]   Cooperation branches swapped: {new_sample['num_cooperation_branches_swapped']}")
    thread_safe_print(f"  [{sample_idx + 1}v{variant_idx + 1}]   Total branches swapped: {new_sample['num_total_branches_swapped']}")
    thread_safe_print(f"  [{sample_idx + 1}v{variant_idx + 1}]   Cost: ${total_cost:.4f}")
    
    return new_sample, total_cost
...
def process_sample_wrapper(args):
    ...
    (sample, model_config, sample_idx, total_samples, 
     variant_idx, total_variants, base_seed) = args
    try:
        new_sample, cost = swap_all_branches_in_sample(
            sample,
            model_config,
            sample_idx,
            total_samples,
        ...
        return sample_idx, variant_idx, new_sample, cost, None
    except Exception as e:
...
def process_single_sample(
    ...
    for variant_idx in variants_to_generate:
        ...
        _, _, new_sample, cost, error = process_sample_wrapper(args)
...
def main():
    """Main function to generate semantic duplicates with ALL branches swapped."""
    # Disable cache to force regeneration
    cache.disable()
    
    parser = argparse.ArgumentParser(
        description="Level 2: Generate Team Allocation semantic duplicates by swapping ALL branches."
    )
    ...
    print("="*80)
    print("LEVEL 2: GENERATING TEAM ALLOCATION SEMANTIC DUPLICATES")
    print("        (Swapping ALL branches - 6 skill + 3 cooperation)")
    print("="*80)
    print(f"\nInput: {input_file}")
    print(f"Base seed: {base_seed} (for reproducibility)")
    print("\nThis script swaps ALL branches per sample:")
    print("  - 6 skill branches (3 people × 2 tasks)")
    print("  - 3 cooperation branches (3 pairs)")
    print("  - Same skill/cooperation levels preserved")
    print("  - New narrative justifications generated")
    ...
    if resuming:
        ...
        output_folder_name = f"team_allocation_level2_samples-{num_samples_actual}_variants-{num_variants_per_sample}_{run_datetime}"
        output_folder = OUTPUT_FOLDER / output_folder_name
        output_folder.mkdir(parents=True, exist_ok=True)
        output_file = output_folder / f"team_allocation_level2_samples-{num_samples_actual}_variants-{num_variants_per_sample}.json"
        metadata_file = output_folder / "run_metadata.txt"
    ...
    if dry_run:
        ...
        # Level 2: ~9 branches × ~2 LLM calls per branch + 2 for story = ~20 calls per variant
        # Average prompt: ~1500 tokens, completion: ~500 tokens per call
        avg_prompt_tokens_per_variant = 20 * 1500  # 30000 tokens
        avg_completion_tokens_per_variant = 20 * 500  # 10000 tokens
        ...
        # Count branches in first few samples for verification
        skill_counts = []
        coop_counts = []
        for sample in samples_to_process[:min(10, num_samples_actual)]:
            ...
            coop_branches = identify_cooperation_branches(tree_json)
            skill_counts.append(len(skill_branches))
            coop_counts.append(len(coop_branches))
        
        avg_skill = sum(skill_counts) / len(skill_counts) if skill_counts else 0
        avg_coop = sum(coop_counts) / len(coop_counts) if coop_counts else 0
        avg_total = avg_skill + avg_coop
        ...
        print(f"\nBranch Analysis (sampled from first {len(skill_counts)} samples):")
        print(f"  Average skill branches per sample: {avg_skill:.1f}")
        print(f"  Average cooperation branches per sample: {avg_coop:.1f}")
        print(f"  Average total branches per sample: {avg_total:.1f}")
        print(f"\nEstimated Costs (ALL branches swapped):")
        ...
        print(f"\nComparison to Level 1 (1 branch):")
        print(f"  Level 2 is ~{avg_total:.0f}× more branches = ~{avg_total:.0f}× higher cost")
    ...
    # Build per-sample missing variant list for resume
    ...
    total_skill_swapped = sum(s.get('num_skill_branches_swapped', 0) for s in final_results)
    total_coop_swapped = sum(s.get('num_cooperation_branches_swapped', 0) for s in final_results)
    total_branches_swapped = sum(s.get('num_total_branches_swapped', 0) for s in final_results)
...
Run Information:
  ...
  Script: level2_team_allocation_swap_all_branches.py
  Resumed: {resuming}
...
Results:
  ...
  Total Branches Swapped: {total_branches_swapped}
    - Skill Branches: {total_skill_swapped}
    - Cooperation Branches: {total_coop_swapped}
...
Description:
  Level 2 Team Allocation semantic duplicates: swap ALL branches (6 skill + 3 cooperation).
  The skill LEVELS and cooperation LEVELS are preserved, only the narrative justifications change.

Methodology:
  1. Identifies ALL skill branches (3 people × 2 tasks = 6 branches)
  2. Identifies ALL cooperation branches (3 pairs = 3 branches)
  3. COPIES the EXACT structure of each branch (preserving complexity)
...
Output Format:
  ...
  - swapped_branches: Dict with full old/new data:
      ...
      - cooperation: [{{person1, person2, coop_level, old_branch, new_branch}}, ...]
  - num_skill_branches_swapped: Number of skill branches swapped
  - num_cooperation_branches_swapped: Number of cooperation branches swapped
  - num_total_branches_swapped: Total branches swapped
  - modification_type: 'full_branch_swap'
...
Complexity Preservation:
  ...
  - Uses copy_branch_structure_for_skill() and copy_branch_structure_for_cooperation()
  - Complexity metrics logged for verification

Answer Preservation:
  - Matrix (skills and cooperation values) is NEVER modified
  - ALL branch justifications are swapped but LEVELS stay the same
...
"""
    ...
    print(f"    - Skill branches: {total_skill_swapped}")
    print(f"    - Cooperation branches: {total_coop_swapped}")
    ...
    print(f"\n📝 Each variant has:")
    print(f"  - ALL 9 branches swapped (6 skill + 3 cooperation)")
    print(f"  - Same skill/cooperation levels preserved")
